...
contract DatahotpotMarketplace is ReentrancyGuard {
    ...
    // Price to list the data into the marketplace
    uint256 listingPrice = 0.001 ether;

    // Price charged to transfer data NFTs to friends
    ...
    modifier onlyProductSeller(uint256 id) {
        require(
            idToMarketItem[id].owner == address(0) && idToMarketItem[id].seller == msg.sender, 
            "Only product can do this operation");
        _;
    }
    ...
    function updateMarketItemPrice(uint256 id, uint256 newPrice) public payable onlyProductSeller(id) {
    ...
    function createMarketSale(address nftContract, uint256 itemId, uint256 price) public payable nonReentrant {
        uint256 itemPrice = idToMarketItem[itemId].price;
        uint256 tokenId = idToMarketItem[itemId].tokenId;
        require(itemPrice == price, "Please submit the asking price in order to complete the purchase");

        idToMarketItem[itemId].seller.transfer(price);

        // The buyer pays the royalty fee to the owner of the NFT
        (address receiver, uint256 royaltyAmount) = ERC2981(nftContract).royaltyInfo(tokenId, price);
        ...
        // Regards the marketplace with listingPrice
    ...
    function fetchMarketItems() public view returns (MarketItem[] memory) {
        ...
        uint256 unsoldItemsCount = _itemsIds.current() - _itemsSold.current() - _itemsDeleted.current();
